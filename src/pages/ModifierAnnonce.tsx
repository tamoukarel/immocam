...
export function ModifierAnnonce() {
  ...
  async function enregistrer() {
    ...
    if (!quartier.trim()) {
      afficherToast(t('modifier.erreurQuartier'))
      return
    }
    if (!prix || Number(prix) <= 0) {
      afficherToast(t('modifier.erreurPrix'))
      return
    }
    ...
    try {
      ...
      afficherToast(t('modifier.succes'))
      navigate('/profil/mes-annonces')
    } catch {
      afficherToast(t('modifier.echec'))
    } finally {
      setEnregistrement(false)
    }
  }

  if (annonce === undefined) return <p className="text-center text-slate-400 py-16">{t('modifier.chargement')}</p>
  if (annonce === null) return <p className="text-center text-slate-500 py-16">{t('modifier.introuvable')}</p>

  return (
    <div className="md:max-w-lg md:mx-auto">
      <PageHeader titre={t('modifier.titre')} sousTitre={t('modifier.sousTitre')} retourVers="/profil/mes-annonces" />
      <div className="px-5 pb-8 flex flex-col gap-3">
        <div>
          <label className="fl">{t('publier.champ.type')}</label>
        ...
        <div className="flex gap-2.5">
          <div className="flex-1 min-w-0">
            <label className="fl">{t('publier.champ.ville')}</label>
          ...
          <div className="flex-1 min-w-0">
            <label className="fl">{t('publier.champ.quartier')}</label>
            <input value={quartier} onChange={(e) => setQuartier(e.target.value)} className="fi" />
          </div>
        </div>
        <div className="flex gap-2.5">
          <div className="flex-1 min-w-0">
            <label className="fl">{t('publier.champ.pieces')}</label>
          ...
          <div className="flex-1 min-w-0">
            <label className="fl">{t('publier.champ.prix')}</label>
        ...
        <div className="flex gap-2.5">
          <div className="flex-1 min-w-0">
            <label className="fl">{t('publier.champ.niveau')}</label>
          ...
          <div className="flex-1 min-w-0">
            <label className="fl">{t('publier.champ.distanceRoute')}</label>
        ...
        <div>
          <label className="fl">{t('modifier.champ.statut')}</label>
          <select value={statut} onChange={(e) => setStatut(e.target.value as StatutAnnonce)} className="fs">
            <option value="dispo">{t('modifier.statut.disponible')}</option>
            <option value="loue">{t('modifier.statut.dejaPris', { statut: libellePris(type === 'vente' ? 'vente' : 'location', lang).toLowerCase() })}</option>
          </select>
        </div>
        <div>
          <label className="fl">{t('publier.champ.description')}</label>
          <textarea value={description} onChange={(e) => setDescription(e.target.value)} className="fi resize-none h-[65px]" />
        </div>
        <div>
          <label className="fl">
            {t('modifier.champ.photosMax')} <span className="text-teal">{t('publier.max', { n: MAX_PHOTOS })}</span>
          </label>
          <div className="flex gap-1.5 overflow-x-auto no-scrollbar pb-1 min-h-[34px] items-center">
            {photos.length === 0 && nouvellesPhotos.length === 0 && <span className="text-xs text-slate-400">{t('modifier.aucunePhoto')}</span>}
            {photos.map((p) => (
          ...
          <label className="mt-2 w-full border-2 border-dashed border-brand-blue rounded-xl p-2.5 bg-blue-light text-brand-blue text-xs font-bold font-heading text-center block cursor-pointer">
            ...
            📷 {t('publier.ajouterPhotos')}
          </label>
        </div>
        <div>
          <label className="fl">
            {t('modifier.champ.videoOpt')} <span className="text-teal">{t('publier.videoOptionnel')}</span>
          </label>
          {nouvelleVideo ? (
            ...
            <label className="border-2 border-dashed border-brand-blue rounded-xl p-4 bg-blue-light text-center block cursor-pointer">
              ...
              <div className="text-xs font-bold text-brand-blue">{t('modifier.ajouterVideo')}</div>
            </label>
          )}
        </div>
        <div>
          <label className="fl">{t('modifier.champ.whatsapp')}</label>
        ...
        <div className="flex gap-2.5 mt-1">
          <button onClick={enregistrer} disabled={enregistrement} className="btn-next disabled:opacity-60">
            {enregistrement ? t('modifier.enregistrement') : t('modifier.enregistrer')}
          </button>
